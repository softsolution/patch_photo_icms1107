...
/* ========================================================================== */
    ...
    if ($do=='upload_photo') {
        ...
        $album = $inDB->getNsCategory('cms_photo_albums', $album_id);
        ...
        $inUploadPhoto = $model->initUploadClass($album);
        $inUploadPhoto->input_name    = 'qqfile';

        $ext = 'jpg,png,gif,jpeg,JPG,PNG,GIF,JPEG,bmp';
        $result = $model->initUploadClass($album)->upload('qqfile', $ext);
        ...
        if ($result['success']) {
            ...
            $photo['file']      = $result['imageurl'];
            $photo['title']     = $photo['title'] ? $photo['title'] . $last_id : $result['name'];
...
/* ========================================================================== */
    ...
    if ($do=='del_photo') {
        ...
        $photo_id = cmsCore::request('photo_id', 'int');

        if (!$photo_id) { cmsCore::halt(); }
