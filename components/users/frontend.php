...
/* ========================================================================== */
    ...
    if ($do=='upload_photo') {
        ...
        if (( $model->config['photosize'] > 0 ) && !$inUser->is_admin) {
            $photo_cnt = $model->getUserPhotoCount($user_id);
            if($photo_cnt >= $model->config['photosize']) {
                header("HTTP/1.1 500 Internal Server Error");
                exit(0);
            }
            //если это последняя фотография
            if($model->config['photosize'] == $photo_cnt + 1){
                $is_limit = true;
            }
        }

        $inUploadPhoto = $model->initUploadClass();
        $inUploadPhoto->input_name    = 'qqfile';

        $ext = 'jpg,png,gif,jpeg,JPG,PNG,GIF,JPEG,bmp';
        $result = $inUploadPhoto->upload('qqfile', $ext);
        ...
        if ($result['success']) {
            ...
            $photo_id = $model->addUploadedPhoto($user_id, array('filename' => $result['name'], 'imageurl' => $result['imageurl']));
